  ...
  return (
    <section className="rounded-lg border border-line bg-pine-850/90 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.35)] sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="font-display text-lg tracking-wide text-cream">{t(lang, "projectionTitle")}</h3>
          <p className="mt-0.5 text-[11.5px] text-dim">
            {t(lang, "projectionSubtitle").replace("{0}", r.ratePct.toFixed(1)).replace("{1}", fmtWan(S * 12))}
          </p>
        </div>
        <div className="flex items-center gap-4 font-mono text-[11px] text-mist">
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-[3px] w-5 rounded bg-jade" /> {t(lang, "assetCurve")}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block w-5 border-t-2 border-dashed border-gold" /> {t(lang, "freedomLine")}
          </span>
        </div>
      </div>
      ...
      <div className="relative mt-4">
        ...
        >
          ...
          {finiteReq && (
            <g>
              ...
              <text x={W - PAD.r} y={y(r.required) - 6} textAnchor="end" fontSize="10" fill="#e8b54a" fontFamily="IBM Plex Mono, monospace">
                {t(lang, "freedomLine")} {fmtWan(r.required)}
              </text>
            </g>
          )}
          ...
          {crossPt && (
            <g>
              ...
              >
                {t(lang, "yearsToFreedom").replace("{0}", String(crossPt.year))}
              </text>
            </g>
          )}
        ...
        {hv && (
          ...
          >
            <div className="font-mono text-[10px] text-dim">{lang === "zh" ? `第 ${hv.year} 年` : `Year ${hv.year}`}</div>
      ...
      <p className="mt-3 border-t border-dashed border-line-soft pt-3 text-[12px] leading-relaxed text-mist">
        {r.crossYear !== null ? (
          <>
            {t(lang, "projectionDesc1")} <b className="font-mono text-gold-soft">{r.crossYear} {lang === "zh" ? "年" : "years"}</b>{t(lang, "projectionDesc2")}
            {fmtWan(r.required)}{t(lang, "projectionDesc3")}
          </>
        ) : finiteReq ? (
          <>
            {t(lang, "projectionDesc4")}<b className="font-mono text-coral">{lang === "zh" ? "40 年内" : "within 40 years"}</b>{t(lang, "projectionDesc5")}
          </>
        ) : (
          <span className="text-coral">{t(lang, "projectionDesc6")}“跑赢印钞机”的问题。</span>
        )}
      </p>
    </section>
  );
